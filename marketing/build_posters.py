...
/* The same lockup the site nav uses — mark and wordmark, no tagline — in dark
   and reversed. Both are rendered from the brand deck's vector art, so neither
   ground needs the logo boxed on a tile any more. */
.tile{{display:inline-block;line-height:0;margin-bottom:calc(var(--u) * 54)}}
.tile img{{display:block;width:auto;height:calc(var(--u) * 168)}}
...
.studio .tile img{{height:calc(var(--u) * 176)}}
...
def main():
    ...
    logos = {
        "studio": data_uri(os.path.join(IMG, "logo-header.png"), "image/png"),
